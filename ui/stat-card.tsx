...
export default function DashboardStatCard({
    ...
    return data.map((d, index) => (
        <div key={index} className="h-[162px] w-full bg-white rounded-2xl py-8 px-4 shadow-md flex justify-between items-center">
            <div className="space-y-2">
                <p className="text-sm text-[#7C7979] font-normal">{d.user}</p>
                <h1 className="text-[#040C13] text-2xl font-bold">{d.amount}</h1>
