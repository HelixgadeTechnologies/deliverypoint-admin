...
export default function OrderSummaryTab({ orderDetails }: SummaryProps) {
  const [openDetailsModal, setOpenDetailsModal] = useState(false);

  if (!orderDetails) return null;

  // for sttaus dependent displays
  const statusExtras: Record<string, JSX.Element> = {
    ...
    <div className="bg-[#FFFBEB] border border-[#FFAC3333] py-[9] px-[13px] rounded-md h-[60px] w-full text-sm text-[#92400E] flex items-start gap-1">
      <Icon icon={"ep:warning-filled"} color="#FFAC33" height={18} width={18} />
      <div>
      ...
      <div className="space-y-4">
        <div className="bg-[#FFFBEB] border border-[#FFAC3333] py-[9] px-[13px] rounded-md h-[60px] w-full text-sm">
          <div className="flex items-center gap-4">
            <Image src={"/stars.svg"} alt="Temporary Rating" height={20} width={80}/>
            <p>{orderDetails.customerInformation.rating?.rateCount}/5</p>
          </div>
          <p className="text-[#92400E] ml-2.5">{orderDetails.customerInformation.rating?.review}</p>
        </div>
        <Divider />
    ...
    <>
      <div className="h-[85px] w-full bg-white p-4 shadow rounded flex justify-between items-center">
        <div className="text-sm">
          <h4 className="text-[#1F1F1F]">{orderDetails.orderId}</h4>
          <p className="text-[#6E747D]">{orderDetails.date}</p>
        </div>
        <div className="space-y-1">
          ...
          <p className="text-sm text-[#6E747D]">₦ {orderDetails.total}</p>
        </div>
      </div>
      <Modal
      ...
      >
        <div className="space-y-3 h-[500px] overflow-y-auto scrollable-modal pr-4">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2.5">
              ...
              <div className="flex gap-2">
                ...
                {/* details */}
                <Heading
                  heading={orderDetails.customerInformation?.name}
                  subtitle={orderDetails.customerInformation.phoneNumber}
                  xs
                  spacing="0"
                />
              </div>
            ...
            <div className="bg-[#F9FAFB] p-3 rounded-lg space-y-2 text-sm">
              ...
              <p className="text-[#7C7979]">
                {orderDetails.customerInformation.address}
              </p>
            </div>
          ...
          {/* divider and button */}
          <Divider />
          <Button content="Close" onClick={() => setOpenDetailsModal(false)} />
        </div>
      </Modal>
    </>
  );
